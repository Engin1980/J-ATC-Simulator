...
public class ClearedToRouteApplication extends CommandApplication<ClearedToRouteCommand> {
  ...
  @Override
  protected IFromAirplane checkCommandSanity(Pilot.Pilot5Command pilot, ClearedToRouteCommand c) {
    if (pilot.getFlight().isArrival() && c.getRoute().getType() == Route.eType.sid)
      return new Rejection("We are arrival, cannot be cleared to SID route.", c);
    else if (!pilot.getFlight().isArrival() && c.getRoute().getType() != Route.eType.sid)
  ...
  @Override
  protected ApplicationResult adjustAirplane(Pilot.Pilot5Command pilot, ClearedToRouteCommand c) {
    pilot.setRoute(c.getRoute(), c.getExpectedRunwayThreshold());
