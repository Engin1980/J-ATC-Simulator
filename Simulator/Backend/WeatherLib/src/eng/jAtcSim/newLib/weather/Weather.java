...
public class Weather {
  ...
  public int getWindSpeedOrWindGustSpeed() {
    if (GAcc.getRnd().nextDouble() < WIND_GUST_PROBABILITY)
      return windGustSpeedInKts;
    else
  ...
  public String toInfoString() {
    ...
    if (GAcc.getSettings().isGetWeatherInfoStringAsMetar()) {
      sb.append("METAR ");
      sb.append(GAcc.getAirportIcao()).append(" ");
      sb.appendFormat("%02d", java.time.LocalDate.now().getDayOfMonth());
      sb.appendFormat("%02d%02dZ ", GAcc.getNow().getHours(), GAcc.getNow().getMinutes());
