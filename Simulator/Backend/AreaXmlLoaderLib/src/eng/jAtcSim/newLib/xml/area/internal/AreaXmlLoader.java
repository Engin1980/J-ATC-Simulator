...
public class AreaXmlLoader extends XmlLoader<Area> {
  ...
  public Area load(XElement source) {
    ...
    this.context.area.icao = SmartXmlLoaderUtils.loadString("icao");
    super.log(0, "... area '%s'", this.context.area.icao);

    this.context.area.navaids = new NavaidList();

    IAreaAcc areaAcc = new LoadingAreaAcc(this.context.area.navaids);
    ContextManager.setContext(IAreaAcc.class, areaAcc);
    SmartXmlLoaderUtils.loadList(
            source.getChild("navaids").getChildren("navaid"),
            this.context.area.navaids,
            new NavaidXmlLoader(this.context)::load
    );

    this.context.area.borders = SmartXmlLoaderUtils.loadList(
            source.getChild("borders").getChildren("border"),
            new BorderXmlLoader(this.context)::load);

    IList<Airport> airports = SmartXmlLoaderUtils.loadList(
            source.getChild("airports").getChildren("airport"),
            new AirportXmlLoader(this.context)::load);

    super.log(0, "... area '%s' loading completed, compiling", this.context.area.icao);

    Area ret = Area.create(
            this.context.area.icao,
            airports,
            this.context.area.navaids,
            this.context.area.borders);

    ContextManager.clearContext(IAreaAcc.class);

    super.log(0, "... area '%s' loading done", this.context.area.icao);
    return ret;
  }
}
